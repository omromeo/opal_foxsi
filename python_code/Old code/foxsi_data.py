...
ellp_flg       = 0                                     # If set to 1, will fit ellipse to points
team           = "Marshall"
filetype       = '.png'                                # File Type
file           = "POS"+str(pos)+"_RUN"+str(run)        # Original file name
dirf           = "C:/Users/Orlando/Downloads/OPAL_FOXSI/"+team+"/Images_08-04-23/"
dirfile        = dirf+file+filetype                    # File Directory
efile          = "Image_Processing/FOXSI4_"+team+"_ellipse_"+file     # Ellipse File
# -----------------------------------------------------------------------------
...
sf = np.array([2.54* 4.0/sz[0], 2.54*4.0/sz[1]])
...
angles   = 0
rings=2
if ellp_flg or not os.path.exists(dirf+efile+".npz"):
    data_matrix = image_proj
    # Find median brightness in each bin by r and theta (polar coordinates)
    centers,axes,angles = opalfox.fitrings(data_matrix,image_bin=image_med,rings=rings,auto=0,rbin=rbin,tbin=tbin,plot=1,minbright=[0,0])
    # Save ellipse files
...
opalfox.plotimage(image_proj,brighten=0,title="FOXSI Optical Alignment")
...
mean_brightness = np.mean(image_proj)
bf              = 100./mean_brightness
image           = cv2.convertScaleAbs(image_proj, 1, bf)
# Set ranges in cm
yrange = np.array([-2,2])*2.54
xrange = np.array([-2,2])*2.54
yr_sim = np.array([-5,5])*.8
xr_sim = np.array([-5,5])*.8
# Find new center
x_off = ((xrange[1]-xrange[0])/2.0)-centers[0,0]*sf[0]
y_off = ((yrange[1]-yrange[0])/2.0)-centers[0,1]*sf[1]

plt.imshow(image_proj,cmap=cmap,extent=[xrange[0]+x_off,xrange[1]+x_off,
                                        yrange[0]+y_off,yrange[1]+y_off])
sim_bright=np.mean(a, axis=-1)
sim_bright[np.where(sim_bright==255)] = np.nan
plt.xlim(-2,2)
plt.ylim(-2,2)
plt.show()
plt.imshow(sim_bright,extent=[xr_sim[0],xr_sim[1],yr_sim[0],yr_sim[1]])
...
# x,y = opalfox.ellipse([0,0],[1,2],np.radians(30),theta=np.linspace(0, 2*np.pi, 100))
# plt.figure()
# plt.plot(x,y)
# plt.plot(np.arange(-1,1,.1),np.arange(-1,1,.1),'-')
# x1,y1 = opalfox.ellipse([0,0],[1,1],np.radians(0),theta=np.linspace(0, 2*np.pi, 100))
# plt.plot(x1,y1)
# plt.grid()
...
# aa = [1,1]
# theta = np.radians(0)
# c[0]**2 + c[1]**2 + aa[0]**2/4*np.cos(theta) + aa[1]**2/4*np.sin(theta)+\
#      aa[0]*np.cos(theta)*(c[0]*np.cos(ang)+c[1]*np.sin(ang)) + \
#      aa[1]*np.sin(theta)*(c[0]*np.sin(ang)+c[1]*np.cos(ang))
